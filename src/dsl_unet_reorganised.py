...
def load_cmip6_zos_data(base_path='../data/zos/'):
    """
    Load and process CMIP6 ZOS (sea surface height) data from multiple scenarios.
    ...
    historical_file = f'{base_path}cmip6_zos_1958_2014_fill_diststen.mat'
    ...
    for scenario in ssp_scenarios:
        # Load scenario data
        scenario_file = f'{base_path}cmip6_zos_{scenario}_2015_2022_fill_diststen.mat'
...
def load_oras5_zos_data(base_path='../data/zos/'):
    """
    Load and process ORAS5 ZOS (sea surface height) data, including historical data
    ...
    # Load historical data (1958-2014)
    historical_file = f'{base_path}oras5_zos_1958_2014_fill_diststen.mat'
    oras5_historical = np.array(scipy.io.loadmat(historical_file)['oras5_ad_sten'])
    
    # Load future data (2015-2022)
    future_file = f'{base_path}oras5_zos_2015_2022_fill_diststen.mat'
...
def data_minus_mean(data):
    oras5_mean1=np.array(scipy.io.loadmat('../data/zos/oras5_historical_zos_1958_2020_mean.mat')['oras5_mclim'])
...
def main():
    ...
    if gpus:
        try:
            ...
            print(e)
    # load cmip6 data
    cmip6_data = load_cmip6_zos_data()
    # load ORAS5 data
    oras5_data = load_oras5_zos_data()
    # remove mean
    ...
    plt.title('UNet SSH loss',size=28)
    plt.ylabel('loss',size=28)
    plt.xlabel('epoch',size=28)
    plt.xticks(size=20)
    plt.yticks(size=20)
    plt.legend(['SSH train losss','SSH Validation loss'], loc='upper right')
    plt.savefig('../output/ssh_train_mse_loss.jpg')
    plt.close()


    scenarios = ['ssp126', 'ssp245', 'ssp370', 'ssp585']
    input_path_template = '../data/zos/cmip6_zos_{scenario}_2023_2100_fill_diststen.mat'
    output_path_template = '../output/unet_zos_{scenario}_2023_2100.npy'
    ...
    scenarios = ['ssp126', 'ssp245', 'ssp370', 'ssp585']
    input_path_template = '../data/zos/cmip6_zos_{scenario}_2015_2022_fill_diststen.mat'
    output_path_template = '../output/unet_zos_{scenario}_2015_2022.npy'
    pix_size = 128  # Replace with the actual pixel size for resizing
    final_size = 85  # Replace with the final size after resizing
    for scenario in scenarios:
        process_ssp_scenario_2023(scenario, model, input_path_template, output_path_template, pix_size, final_size)
